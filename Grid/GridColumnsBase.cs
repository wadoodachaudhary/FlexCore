...
public class GridColumnsBase : ComponentBase
{
    ...
    /// <summary>Moves the column with field <paramref name="fromField"/> so that
    /// it sits immediately before the column with field <paramref name="toField"/>.
    /// No-op when either field can't be found, when they're the same, or when the
    /// column is already in the requested position. Used by GridControl's header
    /// drag-and-drop reordering.</summary>
    internal bool ReorderColumn(string fromField, string toField)
    {
        if (string.IsNullOrEmpty(fromField) || string.IsNullOrEmpty(toField)
            || string.Equals(fromField, toField, StringComparison.Ordinal))
            return false;

        var fromIdx = _columns.FindIndex(c => string.Equals(c.Field, fromField, StringComparison.Ordinal));
        var toIdx   = _columns.FindIndex(c => string.Equals(c.Field, toField,   StringComparison.Ordinal));
        if (fromIdx < 0 || toIdx < 0) return false;

        var moving = _columns[fromIdx];
        _columns.RemoveAt(fromIdx);
        // After removing the moving column, the target index shifts left by one
        // when the move was earlier in the list.
        if (fromIdx < toIdx) toIdx--;
        _columns.Insert(toIdx, moving);
        return true;
    }
